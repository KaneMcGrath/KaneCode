...
internal sealed class GitService : IDisposable
{
    ...
    /// A standard C# / Visual Studio .gitignore file is written into the repository root.
    /// </summary>
    internal bool TryInitializeRepository(string path)
    {
        ...
        WriteGitIgnoreFile(path);

        return true;
    }

    /// <summary>
    /// Writes the standard Visual Studio / C# .gitignore file into the repository root
    /// if one does not already exist.
    /// </summary>
    private static void WriteGitIgnoreFile(string repositoryRoot)
    {
        const string gitIgnoreName = ".gitignore";
        var gitIgnorePath = System.IO.Path.Combine(repositoryRoot, gitIgnoreName);
